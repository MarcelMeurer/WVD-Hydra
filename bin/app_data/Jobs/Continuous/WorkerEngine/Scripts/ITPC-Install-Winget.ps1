...
# Current Version of this script: 2.1
...
if ($mode -eq "InstallApps") {
    ...
    $appsObj | Sort-Object -Property Order | ForEach-Object {
        ...
        $rval=Start-Process -PassThru -FilePath "$($winGetPath)\WinGet.exe" -Wait -ArgumentList "install `"$($_.Id)`" --verbose --disable-interactivity  --accept-source-agreements --source winget" -RedirectStandardOutput "$($LogDir)\WinGetInstaller.$($_.Id).Out.txt" -RedirectStandardError "$($LogDir)\WinGetInstaller.$($_.Id).Warning.txt"
        # one retry
    ...
    $rval=Start-Process -PassThru -FilePath "$($winGetPath)\WinGet.exe" -Wait -ArgumentList "upgrade --all --silent --disable-interactivity --accept-source-agreements --source winget" -RedirectStandardOutput "$($LogDir)\WinGetUpdater.Out.txt" -RedirectStandardError "$($LogDir)\WinGetUpdater.Warning.txt"
    $updated=0
